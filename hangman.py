...
def countdown(tm): 
    while True:
        ...
        for sec in range(tm):
            timer -= 1
            time.sleep(1)
            print("\t\t\t{}:{}".format(timer//60, timer%60) if timer%60 > 9 else "\t\t\t{}:0{}".format(timer//60, timer%60), end='\r')
            if stop_threads:
                break
        print("You ran out of time!\n...click on any letter to exit")
        break
...
def hangman(drawings, num):
    ...
    stop_threads = False
    countdown_thread = threading.Thread(target=countdown, args=(tm,))
