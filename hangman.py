...
def countdown(tm): 
    while True:
        ...
        for sec in range(tm): # loop n times based on the argument
            ...
            if stop_threads: # if flag equals True, kill the thread
                break
        print("You ran out of time!\n...click on any letter to exit") # print message when loop ends
        break
...
def hangman(drawings, num):
    ...
    stop_threads = False # stop flag inicially False
    countdown_thread = threading.Thread(target=countdown, args=(tm,)) # create thread
    ...
    while word_fill != list(word) and timer > 0: # code block will run unless the word is found and there is still time
        ...
        if timer == 0: # check time
            stop_threads = True # stop the clock
            ...
            players[list(players.keys())[turn-1]] += 1  # sums one point to player x
